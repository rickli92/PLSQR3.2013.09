...
#ifdef HAVE_PAPI
  if (dousepapi && GPTL_PAPIstart (t, &row_ptr->aux) < 0)
...
#ifdef HAVE_PAPI
  if (dousepapi && GPTL_PAPIstop (t, &row_ptr->aux) < 0)
...
#ifdef HAVE_PAPI
      memset (&row_ptr->aux, 0, sizeof (row_ptr->aux));
#endif
    }
  }
...
int GPTLpr (const long long id)   /* output file will be named "timing.<id>" */
{
...
// modified by HH<<
//  char dir[256];
//  sprintf(dir, "NP%d", id/1000000);
//  char cmd[256];
//  sprintf(cmd, "mkdir %s", dir);
////  MPI_Barrier(MPI_COMM_WORLD);
////  if(id%1000000==0)//only proc==0 make direcotry
//	system(cmd);
  ...
  */
  ...
  if (iam == 0) {
    ...
    */
    ...
    for (row_ptr = timers[0]->next; row_ptr; row_ptr = row_ptr->next) {

      /* First, master gathers his own stats */

      get_threadstats (row_ptr->name, &summarystats);

      /* Broadcast a message to slaves asking for their results for this timer */

      ret = MPI_Bcast (row_ptr->name, MAX_CHARS+1, MPI_CHAR, 0, comm);
	...
	if (summarystats_slave.count > 0)   /* timer found in slave */
      ...
      fprintf (fp, "%s", row_ptr->name);
      extraspace = max_name_len[0] - strlen (row_ptr->name);
      for (n = 0; n < extraspace; ++n)
	fprintf (fp, " ");
		      ...
		      Summarystats *summarystats)
{
  ...
  */
  ...
  for (t = 0; t < GPTLnthreads; ++t) {
    if ((row_ptr = getentry (hashtable[t], name, &indx))) {

      summarystats->count += row_ptr->count;

      if (row_ptr->wall.accum > summarystats->wallmax) {
	summarystats->wallmax   = row_ptr->wall.accum;
	summarystats->wallmax_t = t;
      }

      if (row_ptr->wall.accum < summarystats->wallmin || summarystats->wallmin == 0.) {
	summarystats->wallmin   = row_ptr->wall.accum;
	summarystats->wallmin_t = t;
      }
#ifdef HAVE_PAPI
      for (n = 0; n < nevents; ++n) {
	double value;
	if (GPTL_PAPIget_eventvalue (eventlist[n].namestr, &row_ptr->aux, &value) != 0) {
...
#ifdef HAVE_PAPI
  GPTL_PAPIquery (&row_ptr->aux, papicounters_out, maxcounters);
#endif
  return 0;
}
		       ...
		       long long *papicounters_out)
{
  ...
  */
  ...
  GPTL_PAPIquery (&row_ptr->aux, papicounters_out, 999);
#endif
  return 0;
}
...
#ifdef HAVE_PAPI
  return GPTL_PAPIget_eventvalue (eventname, &row_ptr->aux, value);
#else
				     ...
				     int *indx)                  /* hash index */
{
  ...
  /*
  ** hashFun index is timer address modulo the table size
			       ...
			       int *indx)                  /* hash index */
{
  ...
  /* hashFun value is sum of chars times their position index */
